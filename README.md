
  # Hi, I'm Christopher Landaverde

  **Marketing Engineer** — I build systems at the intersection of data engineering and growth strategy.

  Specializing in analytics infrastructure, attribution modeling, and privacy-preserving tracking for B2B companies with
   complex measurement challenges.

  [![Website](https://img.shields.io/badge/focosys.io-000?style=for-the-badge&logo=google-chrome&logoColor=white)](https
  :// www.focosys.io)
  
  [![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://
  www.linkedin.com/in/christopherlandaverde)

  ---

  ### What I Do

  - **Analytics Engineering** — GA4, GTM, conversion tracking, and measurement architecture
  - **Attribution & MMM** — Multitouch attribution modeling for media buying teams
  - **Privacy & Consent** — Tracking solutions that respect user privacy
  - **Tooling** — Building internal tools that bridge technical infrastructure with business workflows

  ### Tech I Work With

  ![GA4](https://img.shields.io/badge/GA4-E37400?style=flat-square&logo=google-analytics&logoColor=white)
  ![GTM](https://img.shields.io/badge/GTM-246FDB?style=flat-square&logo=google-tag-manager&logoColor=white)
  ![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
  ![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black)
  ![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
  ![Tailwind](https://img.shields.io/badge/Tailwind-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white)

  ### Featured Projects

  - **[ConsentFlow Debugger](https://github.com/ChristopherLandaverde/ConsentFlow-Debugger)** — Chrome extension for
  validating consent implementations
  - **[Hoagie](https://github.com/ChristopherLandaverde/hoagie)** — Excel-based media planning toolkit built with
  Office.js
  - **[Rabbit](https://github.com/ChristopherLandaverde/rabbit)** — FastAPI service for analyzing marketing touchpoint
  data and applying attribution models
  - **[AdLinter](https://adlinter.vercel.app/)** — Free, privacy-first audit tool for GTM + Google
  Ads that surfaces tracking issues before they cost you money
